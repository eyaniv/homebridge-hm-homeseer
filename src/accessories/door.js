...
// Same value mapping as WindowCovering: 0=closed, 1-98=position, 99=open
const toHk = (hs) => hs >= 99 ? 100 : Math.max(0, Math.round(hs));
const toHs = (hk) => hk >= 100 ? 99 : Math.max(0, Math.round(hk));

function createDoorAccessory(platform, accessory, device) {
  const { Service, Characteristic } = platform.api.hap;
  const ref = device.ref;

  let service = accessory.getService(Service.Door);
  if (!service) service = accessory.addService(Service.Door, accessory.displayName);
...
module.exports = { createDoorAccessory };
