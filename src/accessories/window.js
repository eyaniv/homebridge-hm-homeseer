...
function createWindowAccessory(platform, accessory, device) {
  const { Service, Characteristic } = platform.api.hap;
  const ref = device.ref;

  let service = accessory.getService(Service.Window);
  if (!service) service = accessory.addService(Service.Window, accessory.displayName);
...
module.exports = { createWindowAccessory };
